...
transe = torch.nn.DataParallel(transe)
...
transr = torch.nn.DataParallel(transr)
...
trainer.run()
parameters = transe.module.get_parameters()
transe.module.save_parameters("./result/transr_transe.json")

# train transr
transr.module.set_parameters(parameters)
trainer = Trainer(model = model_r, data_loader = train_dataloader, train_times = 1000, alpha = 1.0, use_gpu = True)
trainer.run()
transr.module.save_checkpoint('./checkpoint/YAGO/YAGO_TransR.ckpt')

# test the model
transr.module.load_checkpoint('./checkpoint/YAGO/YAGO_TransR.ckpt')
