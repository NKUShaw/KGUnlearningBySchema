...
# train the model
transd.load_checkpoint('./checkpoint/YAGO/YAGO_TransD.ckpt')
trainer = Trainer(model = model, data_loader = train_dataloader, train_times = 100, alpha = 1e-4, use_gpu = True)
trainer.run_grad_ascent()
transd.save_checkpoint('./checkpoint/GradAscent_Edges_TransD_YAGO.ckpt')

# test the model
transd.load_checkpoint('./checkpoint/GradAscent_Edges_TransD_YAGO.ckpt')
